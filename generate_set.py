...
for instrument in instruments:
    ...
    with ThreadPoolExecutor(max_workers = 8) as executor:
        ...
        def renderBlock(i, f):
            ...
            # each thread has a corresponding tf session
            thread_id = threading.get_ident()
            if thread_id not in sessions:
                with lock:
                    sessions.update({thread_id: tf.Session()})
            ...
            for (j, start) in enumerate(np.linspace(0, len(waveform)-num_samples-1, num=block_size)):
                ...
                result[:,:,layer] = sessions[thread_id].run(logmel(waveform[start:start+num_samples]))
                # print progress
                with lock:
                    done += 1
                    #print('\rspectrogram', done,'/',spectrogram_count,'done', flush=True, end="")
                    print('spectrogram', done,'/',spectrogram_count,'done', flush=True)
        ...
        for (i, f) in enumerate(files):
            if i*block_size<spectrogram_count:
                executor.submit(renderBlock, i, f)
        print('futures generated')
